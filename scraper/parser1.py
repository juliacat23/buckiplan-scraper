...
class SolusParser(object):
    ...
    def __init__(self):
        ...
        # Prefer lxml, fall back to built in parser1
        try:
            bs4.BeautifulSoup("", self._souplib)
        except bs4.FeatureNotFound as e:
            logging.warning(u"Not using {0} for parsing, using builtin parser1 instead".format(self._souplib))
            self._souplib = "html.parser1"

    def update_html(self, text):
        """Feed new data to the parser1"""
        self.soup = bs4.BeautifulSoup(text, self._souplib)

    def dump_html(self):
        """Dumps the contents of the parser1 to a file"""
    ...
    def login_solus_link(self):
        ...
        #link = self.soup.select("a", attr = {"title": "Student Center"})
        link = self.soup.find(title="Student Center")
        ...
        if not link:
            ...
            print("This is type of link.get method: " + str(type(link.get("href"))))
            return str(link.get("href"))

    def get_input_vars(self):

        print('getting lt and execution...')
        print(self.soup.title)
        #post_lt = self.soup.find("input")
        lt_tag = self.soup.select('input[name="lt"]')[0]
        execution_tag = self.soup.select('input[name="execution"]')[0]

        lt_value = lt_tag['value']
        execution_value = execution_tag['value']


        #post_execution = self.soup.find("input", attr = {"name": "execution"})
    ...
    def section_attrs_at_index(self, index):
        ...
        for x in range(0, len(values), 5):
            ...
            if temp_inst and temp_inst not in NON_INSTRUCTORS:
                ...
                print ('Printing list in section parser')
                print (lis)
                for i in range(0, len(lis), 2):
                    last_name = lis[i].strip()
                    #other_names = lis[i+1].strip()
                    #instructors.append(u"{0}, {1}".format(last_name, other_names))
                    #instructors.append(u"{0}, {1}".format(last_name))
                    instructors.append(u"{0}".format(last_name))
            ...
            term_start = datetime.strptime(m.group(1), "%m/%d/%Y").date() if m else None
            term_end = datetime.strptime(m.group(2), "%m/%d/%Y").date() if m else None
